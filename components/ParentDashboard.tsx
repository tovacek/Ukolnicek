...
const ParentDashboard: React.FC = () => {
  ...
  useEffect(() => {
      if (currentUser && activeTab === 'settings') {
          ...
          // Note: We cannot retrieve the login password for security display usually, but for this demo if it's in context:
  ...
  const handleProfileSettingsSave = async (e: React.FormEvent) => {
      ...
      // Update Name/Avatar via updateChild
          ...
          <div className="space-y-8 animate-fade-in max-w-6xl mx-auto">
                    ...
                    <div key={child.id} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 relative group flex flex-col justify-between">
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-4 mb-6">
                              <div className="bg-indigo-50 p-3 rounded-lg text-center"><div className="text-xs text-indigo-400 font-semibold uppercase">Body</div><div className="text-2xl font-bold text-indigo-700">{child.points}</div></div>
                              <div className="bg-emerald-50 p-3 rounded-lg text-center"><div className="text-xs text-emerald-600 font-semibold uppercase">Kasička</div><div className="text-2xl font-bold text-emerald-700">{child.balance} Kč</div></div>
                          </div>
                        </div>
                ...
                <div className="bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl flex flex-col items-center justify-center p-6 min-h-[300px] hover:border-indigo-400 hover:bg-indigo-50/30 transition-colors cursor-pointer">
                    {isAddingChild ? (
                         <form onSubmit={handleAddChild} className="w-full flex flex-col items-center gap-4">
                            ...
                            <input type="text" placeholder="Jméno" value={newChildName} onChange={e => setNewChildName(e.target.value)} className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800" autoFocus />
                        ...
                        <button onClick={() => setIsAddingChild(true)} className="flex flex-col items-center gap-3 text-slate-400 hover:text-indigo-600"><div className="p-4 bg-white rounded-full shadow-sm"><Plus size={32}/></div><span className="font-medium">Přidat dítě</span></button>
                    )}
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
               <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:justify-between sm:items-center bg-slate-50 gap-2">
                    ...
                    <span className="text-sm font-medium text-slate-500 bg-white px-3 py-1 rounded-full border border-slate-200 shadow-sm">Celkem vyplaceno: <span className="text-emerald-600 font-bold">{getTotalPayout()} Kč</span></span>
               </div>
               {payoutHistory.length === 0 ? <div className="p-8 text-center text-slate-400 text-sm italic">Zatím neproběhly žádné výplaty.</div> : (
                   <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider text-xs"><tr><th className="p-4 font-medium">Datum</th><th className="p-4 font-medium">Dítě</th><th className="p-4 font-medium text-right">Částka</th></tr></thead>
                                ...
                                {payoutHistory.map(record => {
                                    ...
                                    return (
                                        <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="p-4 font-bold text-slate-700 flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 text-xs font-bold">{childName.charAt(0)}</div>{childName}</td>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in max-w-6xl mx-auto">
              <div className="lg:col-span-1 space-y-6">
                      ...
                      <form onSubmit={handleCreateTask} className="space-y-4">
                          <div>
                              ...
                              <div className="flex gap-2 overflow-x-auto pb-2">
                                  <button type="button" onClick={() => setSelectedChildId(ALL_CHILDREN_ID)} className={`flex-shrink-0 px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-1 ${selectedChildId === ALL_CHILDREN_ID ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}><Users size={14} /> Všem</button>
                                  {children.map(child => (
                                      <button key={child.id} type="button" onClick={() => setSelectedChildId(child.id)} className={`flex-shrink-0 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${selectedChildId === child.id ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{child.name}</button>
                                  ))}
                              </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              <div><label className="block text-sm font-medium text-slate-600 mb-1">Body</label><input type="number" min="0" value={newTaskPoints} onChange={e => setNewTaskPoints(Number(e.target.value))} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 text-slate-800"/></div>
                              <div><label className="block text-sm font-medium text-slate-600 mb-1">Kč</label><input type="number" min="0" value={newTaskMoney} onChange={e => setNewTaskMoney(Number(e.target.value))} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 text-slate-800"/></div>
                          </div>
                          <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                                <div className="flex items-center gap-2 mb-2"><input type="checkbox" id="isRecurring" checked={isRecurring} onChange={(e) => setIsRecurring(e.target.checked)} className="rounded text-indigo-600 focus:ring-indigo-500"/><label htmlFor="isRecurring" className="text-sm font-medium text-slate-700 flex items-center gap-1"><Repeat size={14} /> Opakovat pravidelně</label></div>
                  ...
                  <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-6 rounded-xl shadow-lg text-white">
                      <div className="flex items-center gap-2 mb-2"><Sparkles className="text-yellow-300" /><h3 className="font-bold text-lg">AI Inspirace</h3></div>
                      <p className="text-indigo-100 text-sm mb-4">Nech Gemini vymyslet úkoly na míru.</p>
                      <input type="text" placeholder="Co dítě baví? (např. Lego, dinosauři)" value={interests} onChange={e => setInterests(e.target.value)} className="w-full p-2 rounded-lg bg-white/20 border border-white/30 placeholder-indigo-200 text-white mb-3 focus:outline-none focus:bg-white/30"/>
                      <button onClick={handleGenerateIdeas} disabled={isGenerating || selectedChildId === ALL_CHILDREN_ID} className="w-full py-2 bg-white text-indigo-600 rounded-lg font-bold hover:bg-indigo-50 disabled:opacity-70">{isGenerating ? 'Generuji...' : selectedChildId === ALL_CHILDREN_ID ? 'Vyber konkrétní dítě' : 'Navrhnout úkoly'}</button>
                      {generatedIdeas.length > 0 && (<div className="mt-4 space-y-2">{generatedIdeas.map((idea, idx) => (<div key={idx} onClick={() => applyIdea(idea)} className="bg-white/10 p-2 rounded hover:bg-white/20 cursor-pointer text-sm border border-white/10"><div className="font-bold">{idea.title}</div><div className="text-xs opacity-80">{idea.suggestedPoints} bodů</div></div>))}</div>)}
                  </div>
              </div>
              ...
              <div className="lg:col-span-2">
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                      <div className="divide-y divide-slate-100">
                          ...
                          {tasks.filter(t => (t.status === TaskStatus.TODO || t.status === TaskStatus.REJECTED) && (selectedChildId === ALL_CHILDREN_ID || t.assignedToId === selectedChildId)).length === 0 && <div className="p-8 text-center text-slate-400">Žádné aktivní úkoly{selectedChildId !== ALL_CHILDREN_ID && ' pro vybrané dítě'}.</div>}
                      </div>
                  </div>
              </div>
           </div>
             ...
             <div className="max-w-4xl mx-auto animate-fade-in grid grid-cols-1 md:grid-cols-2 gap-8">
                 ...
                 <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 h-fit">
                     <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
                        <Home className="text-indigo-600" /> Rodinný účet
                     </h3>
                     <p className="text-slate-500 text-sm mb-6">Údaje pro přihlášení do aplikace.</p>

                     <form onSubmit={handleFamilySettingsSave} className="space-y-4">
                        <div>
                             <label className="block text-sm font-medium text-slate-600 mb-1">Název rodiny</label>
                             <input 
                                type="text"
                        ...
                        <div>
                             <label className="block text-sm font-medium text-slate-600 mb-1">Heslo</label>
                             <div className="relative">
                                <input 
                                    type={showFamilyPassword ? "text" : "password"}
                                    className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none text-slate-800"
                                    placeholder="Změnit heslo..."
                                    value={familyPassword}
                                    onChange={(e) => setFamilyPassword(e.target.value)}
                                />
                                <button type="button" onClick={() => setShowFamilyPassword(!showFamilyPassword)} className="absolute right-3 top-3 text-slate-400 hover:text-indigo-600">
                        ...
                        <button 
                            type="submit" 
                            className="w-full py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-lg hover:bg-indigo-700 transition-all mt-4"
                        >
                            Uložit rodinné údaje
                        </button>
                     </form>
                 </div>

                 {/* Section 2: Parent Profile Settings */}
                 <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 h-fit">
                     <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
                        <UserCircle className="text-indigo-600" /> Profil rodiče
                     </h3>
                     ...
                     <form onSubmit={handleProfileSettingsSave} className="space-y-4">
                        <div>
                             <label className="block text-sm font-medium text-slate-600 mb-1">Jméno</label>
                             <input 
                                type="text"
                                className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none text-slate-800"
                                value={profileName}
                        ...
                        <div>
                             <label className="block text-sm font-medium text-slate-600 mb-1">PIN profilu (4 čísla)</label>
                             <div className="relative">
                                <input 
                                    type={showProfilePin ? "text" : "password"}
                                    maxLength={4}
                                    className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none text-slate-800 tracking-widest font-mono"
                                    placeholder="####"
                                    value={profilePin}
                                    onChange={(e) => setProfilePin(e.target.value)}
                                />
                                <button type="button" onClick={() => setShowProfilePin(!showProfilePin)} className="absolute right-3 top-3 text-slate-400 hover:text-indigo-600">
                                    {showProfilePin ? <EyeOff size={20}/> : <Eye size={20}/>}
                                </button>
                             </div>
                             <p className="text-xs text-slate-400 mt-1">Slouží pro přepnutí na tento profil.</p>
                        </div>
                        <button 
                            type="submit" 
                            className="w-full py-3 bg-indigo-50 text-indigo-700 font-bold rounded-xl border border-indigo-100 hover:bg-indigo-100 transition-all mt-4"
                        >
                            Uložit profil
                        </button>
                     </form>
                 ...
                 {settingsMessage && (
                     <div className={`col-span-1 md:col-span-2 p-3 rounded-lg text-sm font-bold text-center ${settingsMessage.includes('Chyba') ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                         {settingsMessage}
                     </div>
                 )}
             </div>
