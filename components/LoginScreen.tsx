...
const LoginScreen: React.FC = () => {
  ...
  const [inputProfilePassword, setInputProfilePassword] = useState('');
  ...
  const handleUserClick = (userId: string) => {
    ...
    if (user && user.password) {
      ...
      setInputProfilePassword('');
    } else {
      selectProfile(userId);
    }
  };

  const submitProfilePassword = (e: React.FormEvent) => {
    e.preventDefault();
    if (!authModal.userId) return;
    
    const user = users.find(u => u.id === authModal.userId);
    if (user && user.password === inputProfilePassword) {
  ...
  // If familyId exists (restored from session/context), go to profiles
  React.useEffect(() => {
          ...
          <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-100 flex flex-col">
              <header className="p-6 flex justify-between items-center max-w-7xl mx-auto w-full">
                  ...
                  <button 
                    ...
                    className="px-6 py-2 rounded-full font-bold text-brand-blue hover:bg-blue-50 transition-colors"
                  >
                    Přihlásit se
                  </button>
              </header>
              ...
              <main className="flex-1 flex flex-col items-center justify-center text-center px-4 pb-20">
                  <div className="mb-8 animate-bounce">
                      <div className="inline-block p-6 bg-white rounded-[2rem] shadow-xl shadow-blue-200 transform -rotate-3">
                        <Sparkles size={64} className="text-brand-yellow" />
                      </div>
                  </div>
                  
                  <h2 className="text-5xl md:text-7xl font-display font-bold text-slate-800 mb-6 leading-tight">
                      Domácí povinnosti <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-blue to-brand-green">hrou.</span>
                  </h2>
                  <p className="text-xl text-slate-500 max-w-2xl mb-10 leading-relaxed">
                      Motivujte děti k plnění úkolů pomocí odměn. <br className="hidden md:block"/>Jednoduché, zábavné a vytvořené pro moderní rodiny.
                  </p>

                  <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
                      <button 
                        onClick={() => setView('REGISTER')}
                        className="flex-1 py-4 bg-brand-dark text-white rounded-2xl font-bold text-lg shadow-lg hover:shadow-xl hover:scale-105 transition-all"
                      >
                          Vytvořit novou rodinu
                      </button>
                      <button 
                        onClick={() => setView('LOGIN')}
                        className="flex-1 py-4 bg-white text-brand-dark rounded-2xl font-bold text-lg shadow-lg hover:bg-gray-50 transition-all"
                      >
                          Mám účet
                      </button>
                  </div>
                  ...
                  <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl w-full">
                      <div className="bg-white/60 backdrop-blur-sm p-6 rounded-2xl border border-white shadow-sm">
                          <CheckSquare className="w-10 h-10 text-brand-blue mb-4" />
                          <h3 className="font-bold text-lg mb-2">Přehledné úkoly</h3>
                          <p className="text-slate-500 text-sm">Jednoduchý seznam úkolů s fotkami pro potvrzení.</p>
                      </div>
                      <div className="bg-white/60 backdrop-blur-sm p-6 rounded-2xl border border-white shadow-sm">
                          <Star className="w-10 h-10 text-brand-yellow mb-4" />
                          <h3 className="font-bold text-lg mb-2">Motivace a body</h3>
                          <p className="text-slate-500 text-sm">Sbírání bodů a jejich výměna za skutečné odměny.</p>
                      </div>
                      <div className="bg-white/60 backdrop-blur-sm p-6 rounded-2xl border border-white shadow-sm">
                          <Home className="w-10 h-10 text-brand-green mb-4" />
                          <h3 className="font-bold text-lg mb-2">Pro celou rodinu</h3>
                          <p className="text-slate-500 text-sm">Jeden účet pro rodiče i všechny děti. Vše pohromadě.</p>
                      </div>
                  </div>
              </main>
          </div>
                ...
                <div className="p-8">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <h2 className="text-2xl font-display font-bold text-slate-800">
                            {view === 'LOGIN' ? 'Přihlášení' : 'Registrace'}
                        </h2>
                    ...
                    <form onSubmit={view === 'LOGIN' ? handleLoginFamily : handleRegisterFamily} className="space-y-4">
                        {view === 'REGISTER' && (
                            <div>
                                <label className="block text-sm font-bold text-slate-600 mb-1">Název rodiny</label>
                                <input 
                                    type="text" 
                                    required
                                    className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-blue outline-none text-slate-800"
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-600 mb-1">Email rodiče</label>
                            <input 
                                type="email" 
                                required
                                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-blue outline-none text-slate-800"
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-600 mb-1">Heslo</label>
                            <input 
                                type="password" 
                                required
                                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-blue outline-none text-slate-800"
                        ...
                        <button 
                            type="submit" 
                            disabled={isLoading}
                            className="w-full py-3 bg-brand-blue text-white font-bold rounded-xl shadow-lg hover:bg-blue-600 transition-all active:scale-95 disabled:opacity-70"
                        >
                            {isLoading ? 'Pracuji...' : (view === 'LOGIN' ? 'Vstoupit' : 'Založit účet')}
                        </button>
                    </form>

                    <div className="mt-6 text-center">
                        <p className="text-slate-500 text-sm">
                            {view === 'LOGIN' ? 'Nemáte ještě účet?' : 'Už máte účet?'}
                            <button 
                                onClick={() => setView(view === 'LOGIN' ? 'REGISTER' : 'LOGIN')}
                                className="ml-2 font-bold text-brand-blue hover:underline"
                            >
                                {view === 'LOGIN' ? 'Zaregistrujte se' : 'Přihlaste se'}
                            </button>
                        </p>
                    </div>
                </div>
  ...
  // 3. PROFILE SELECTION (Existing Logic wrapped)
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-indigo-100 flex flex-col items-center justify-center p-4 animate-fade-in">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl overflow-hidden relative">
        <div className="absolute top-4 right-4">
            <button onClick={handleLogoutFamily} className="text-slate-300 hover:text-slate-500 p-2" title="Odhlásit rodinu">
                <LogOut size={20} />
            </button>
        </div>
        ...
        <div className="p-8 text-center">
          ...
          {users.length === 0 ? (
              <div className="py-10">
                  ...
                  <p className="text-slate-400">Načítám rodinu...</p>
              </div>
          ) : (
                ...
                <div className="grid grid-cols-1 gap-3">
                    ...
                    >
                        <div className="flex items-center">
                            <img src={user.avatarUrl} alt={user.name} className="w-12 h-12 rounded-full border-2 border-white shadow-sm object-cover" />
        ...
        {authModal.isOpen && (
            <div className="absolute inset-0 bg-white/90 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-8 animate-fade-in">
                ...
                <form onSubmit={submitProfilePassword} className="w-full max-w-xs">
                    <input 
                        type="password" 
                        autoFocus
                        value={inputProfilePassword}
                        onChange={(e) => setInputProfilePassword(e.target.value)}
